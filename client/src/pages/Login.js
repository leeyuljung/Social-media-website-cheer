...
const Login = () => {
  ...
  const { values, loading, onChange, onSubmit, setLoading } = useForm(loginUser, {
    username: '',
    email: '',
    password: '',
    passwordConfirm: ''
    ...
    <div className="w-[500px] mx-auto mt-12 px-10 py-10 bg-white rounded-lg shadow-lg overflow-hidden" style={{background: 'linear-gradient(to bottom, rgba(255, 190, 111, 0.5), rgba(206, 182, 240, 0.5))'}}>
      ...
      <form className="mt-8" onSubmit={ onSubmit } noValidate>
        ...
        <div className="mt-6 text-sm text-center text-[#585a79] drop-shadow-md tracking-wider">
          <p>Don't have an account yet? <Link to="/register" className="mx-2 text-[#f8f8f2] transition duration-500 hover:text-[#585a79]">Register</Link></p>
        </div>
      </form>
      { 
